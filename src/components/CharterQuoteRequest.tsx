...
export const CharterQuoteRequest = ({ leadData }: CharterQuoteRequestProps) => {
  ...
  const handleRequestQuotes = async () => {
    ...
    try {
      // Extract airport codes (remove city names if present)
      ...
      // Build request body for Aviapages API
      const requestBody = {
        legs: [
          {
            departure_airport: {
        ...
        aircraft: [
          {
            ac_class: "Midsize" // Default - can be made dynamic based on analysis
          }
        ],
        quote_messages: [
          // You would populate this with actual operators from charter search
          // For now, this will need to be configured or selected by user
        ],
        quote_extension: {
      ...
      toast.success('Charter quotes requested successfully!');
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Request Charter Quotes from Aviapages</CardTitle>
          <CardDescription>
            Request quotes from multiple charter operators through Aviapages network
          </CardDescription>
        </CardHeader>
